...
ABOUT_CSS = """
<style>
    :root { --bg-global: #06090d; --text-main: #c9d1d9; --bg-card: #0d1117; --border-main: #30363d; --border-neon: #58a6ff; --border-cyber: #3fb950; --text-white: #fff; --border-sub: #21262d; }
    body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif; background: var(--bg-global); color: var(--text-main); margin: 0; padding: 0; box-sizing: border-box; display: flex; flex-direction: column; min-height: 100vh; justify-content: center; overflow-x: hidden; position: relative; }
    ...
    .about-box { background: var(--bg-card); border: 1px solid var(--border-main); border-radius: 14px; padding: 35px; width: 100%; max-width: 600px; box-shadow: 0 20px 40px rgba(0,0,0,0.3); border-bottom: 4px solid var(--border-neon); border-left: 4px solid var(--border-cyber); box-sizing: border-box; text-align: right; direction: rtl; }
    ...
    .global-footer-bar { width: 100%; text-align: center; padding: 20px 0; border-top: 1px solid var(--border-sub); font-size: 12px; color: #8b949e; font-family: monospace; margin-top: auto; position: relative; z-index: 100; }

    /* 🕹️ حقن وتثبيت معايير الستارة الجانبية المنبثقة دائرياً الموحدة لمنع التداخل البصري */
    .sidebar-overlay { 
...
<body>
    ...
    <!-- الستارة الجانبية مدمجة ومصبوبة حياً داخل نفس ملف صفحة التعريف لمنع التداخل والـ 404 كلياً -->
