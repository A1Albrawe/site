...
<body>
    ...
    <script>
        ...
        const ROW = 20; const COL = 12; const SQ = 20; let score = 0;
        ...
        // ✅ شحن وضبط مصفوفات المكعبات السبعة لإنهاء الـ Syntax Error تماماً
        const PIECES = [
            [[[1,1,1,1]], "cyan"],
            [[[1,1,1],[0,1,0]], "purple"],
            [[[1,1,0],[0,1,1]], "red"],
        ...
        function movePiece(dx, dy) {
            ...
            if(collision()) { activePiece.x -= dx; if(dy === -1) { let rMatrix = activePiece.matrix.map((val, index) => activePiece.matrix.map(row => row[index]).reverse()); activePiece.matrix = rMatrix; } else { activePiece.y -= dy; lockPiece(); } }
            drawPiece();
        }
        ...
        function lockPiece() {
            ...
            for(let r=0; r<ROW; r++) { if(board[r].every(val => val !== "#04060a")) { board.splice(r, 1); board.unshift(new Array(COL).fill("#04060a")); score += 10; document.getElementById("tetrisScore").innerText = score; } }
            drawBoard(); activePiece = getRandomPiece();
        }

        function dropPiece() { movePiece(0, 1); }
        let gameTimer = setInterval(dropPiece, 1000);
