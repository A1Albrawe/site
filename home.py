...
# عزل التنسيقات السيبرانية لحماية بايثون من التعارض النصي للأقواس خارج الصندوق
HOME_TERMINAL_CSS_PART1 = """
<style>
    :root {
...
<style>
    ...
    /* 💻 تثبيت وعزل أبعاد كرت الأجنحة الممتد جغرافياً لمنع الهبوط أو التعليق المكسور */
    .responsive-profile-wrapper { 
        ...
        padding: 45px; box-shadow: 0 25px 55px rgba(0,0,0,0.8), inset 0 0 20px rgba(0,255,102,0.05); 
        border-bottom: 4px solid var(--border-neon); border-right: 4px solid var(--border-cyber);
        box-sizing: border-box; align-items: center; direction: rtl; backdrop-filter: blur(10px); -webkit-backdrop-filter: blur(10px);
        position: relative; z-index: 50; clip-path: polygon(0 0, 97% 0, 100% 4%, 100% 100%, 3% 100%, 0 96%);
    }
...
"""
HOME_TERMINAL_CSS_PART3 = """
<style>
    /* 🕹️ تثبيت وتحقيق الانبثاق الدائري الموحد بأعلى طبقة عرض حظراً للتداخل البصري والهبوط */
    .sidebar-overlay { 
    ...
    @media (max-width: 850px) {
        body { padding: 15px; } .top-nav { max-width: 100%; } .responsive-profile-wrapper { flex-direction: column; align-items: center; padding: 25px; max-width: 440px; clip-path: none !important; }
        .profile-sidebar-zone { flex: none; width: 100%; max-width: 100%; border-left: none; border-bottom: 2px solid var(--border-main); padding-left: 0; padding-bottom: 25px; margin-bottom: 20px; }
        ...
        if os.path.exists(games_dir):
            for filename in sorted(os.listdir(games_dir)):
                if filename.endswith('.txt'):
                    ...
                    # ✅ التطهير البرمجي النهائي الحاسم: صياغة وقفل الفهارس صراحة لمنع عطل الـ 500 كلياً للأبد حياً
...
<head>
    ...
    """ + HOME_TERMINAL_CSS_PART1 + HOME_TERMINAL_CSS_PART2 + HOME_TERMINAL_CSS_PART3 + """
</head>
<body>
